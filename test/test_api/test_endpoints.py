...
# Test veritabanı için veritabanı tablolarını oluşturur
...
def test_api_create_device_data():
    response = client.post(
        ...
        json={
            ...
            "service_point": "API Test SP",
            "mrf": "API Test MRF",
            "parcel": "API Test Parcel",
            "gps": ["API Test GPS1", "API Test GPS2"]  # GPS alanı bir liste olarak güncellendi
        }
    )
    assert response.status_code == 200
    data = response.json()
    assert data["route"] == "API Test Route"
    assert data["gps"] == ["API Test GPS1", "API Test GPS2"]  # Liste olarak kontrol ediliyor
...
def test_api_get_device_data():
    ...
    post_response = client.post(
        ...
        json={
            ...
            "mrf": "API Test MRF",
            "parcel": "API Test Parcel",
            "gps": ["API Test GPS1", "API Test GPS2"]
        }
    )
